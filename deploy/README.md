...
curl -sSL https://raw.githubusercontent.com/rulanup/sub2api/main/deploy/docker-deploy.sh -o docker-deploy.sh
...
> The installer only downloads releases from `rulanup/sub2api` and does not fall back to upstream binaries. If the fork has no GitHub Release, deploy `ghcr.io/rulanup/sub2api:latest` with Docker Compose instead.
...
1. Download the latest release from [GitHub Releases](https://github.com/rulanup/sub2api/releases)
