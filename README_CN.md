...
- **私有账号池** - 用户可添加和管理自己的上游账号，并支持通过 API 地址和密钥快速添加、自动发现模型
- **私有分组** - 用户可将私有账号组织为仅本人可见的分组；私有分组与公共、专属分组隔离，平台不对其调用计费
- **私有模型与渠道** - 模型广场和可用渠道支持在公共资源与用户私有分组提供的模型之间切换
- **多分组调度** - 一个 API Key 可绑定多个同平台分组，按最低成本或最低延迟选择分组，并在分组无可用账号时自动回退
- **渠道价格导入** - 支持从账号白名单或模型映射导入渠道计费模型
- **消费排行榜** - 用户可查看日/周/月消费排名，了解自己在全站的消费位置
- **每日签到** - 用户每日签到领取随机额度，管理员可配置额度范围和开关
- **抽奖活动** - 管理员可配置活动时间、每用户每日次数、全站总次数及加权奖池；奖品支持余额和固定天数的订阅分组权限
- **CC-Switch 集成** - API Key 可直接导入 Claude Code、Codex、OpenCode、Gemini CLI 或 Grok Build，并通过 `/v1/usage` 在 CC-Switch 中查询余额、配额和订阅余量
- **异常使用自动检测** - 检测持续同步中转流量和重复 `cyber_policy` 使用，可自动调整用户 RPM/并发或禁用用户，并为两类检测提供独立的可搜索用户白名单
- **错误定制** - 按 HTTP 状态码、错误关键词和上游平台定制返回状态码及用户可见错误文本，内置 401、429、安全策略预设，并支持全局用户白名单
- **Resend 邮件投递** - 验证码和通知邮件可直接通过 Resend HTTPS API（resend.com）发送，替代传统 SMTP，并支持通过 `GET /domains` 测试连接
- **邮件配置迁移** - 一键将完整邮件配置（SMTP + Resend）导出/导入为 JSON 文件；默认不包含敏感信息，导入时只处理邮件相关设置项
...
- 在 **系统设置 > 网关设置 > 错误定制** 创建规则，可按 HTTP 状态码、响应关键词和上游平台匹配。规则可保留或替换响应状态码，并支持完全替换错误文本。
- 错误定制覆盖 OpenAI、Anthropic、Gemini、Antigravity、Grok、图片接口、流式响应和 WebSocket，且不会改变重试、账号切换、计费或风控证据。
- 全局错误定制白名单中的用户跳过所有可编辑规则，接收上游原始状态码和错误消息；兼容接口仍使用其协议要求的响应外层，必要的安全脱敏仍然生效。
- 多实例部署通过 Redis 同步规则和白名单变更，并定期从 PostgreSQL 校准白名单。

### 抽奖活动与 CC-Switch

- 在 **系统设置 > 功能 > 抽奖活动** 配置活动 ID、标题、开始/结束时间、每用户每日抽奖次数和全站总抽奖次数。修改活动 ID 会开启独立的新一期活动记录。
- 抽奖免费，不检查或扣除用户余额。开奖结果由后端在 PostgreSQL 事务内生成；次数占用、抽奖记录和余额或订阅权限发放同时提交。活动到期或全站次数用尽后自动停止。
- 奖池支持加权余额奖品和固定有效天数的订阅分组奖品。用户已拥有的有效分组奖品会从该用户的当次奖池中排除。
- 在 **API Keys** 页面点击 **导入到 CCS**。OpenAI 分组可选择 Claude Code、Codex 或 OpenCode；其他平台只展示兼容客户端。CC-Switch 每次导入一个客户端，如需多个客户端请分别执行导入。
- 导入配置会启用 CC-Switch 用量查询并定期请求 `GET /v1/usage`，显示钱包余额、API Key 配额、订阅余量或速率限制。OpenCode 使用独立的根查询地址，避免产生重复的 `/v1/v1/usage` 路径。

### Resend 邮件与配置迁移

- 在 **系统设置 > 邮箱设置** 选择投递通道：**SMTP** 或 **Resend API**。Resend 需要在 Resend 控制台创建域名 API Key，且发信域名必须已验证并开启发送。
- Resend 卡片中的 **测试连接** 通过 `GET /domains` 校验 API Key 且不会产生任何发送；仍可使用 **发送测试邮件** 验证收件。
- 邮件发送通道卡片中的 **导出配置** 可将完整邮件配置（SMTP + Resend 全部键）下载为 JSON 文件。默认排除 SMTP 密码与 Resend API Key，仅在迁移到新部署时勾选 **包含敏感信息**。
- **导入配置** 从该 JSON 文件恢复：只应用邮件相关设置键，非邮件键一律忽略，留空的敏感字段保持现有值不变。
...
### 方式一：脚本安装（需要 Fork Release）

一键安装脚本，自动从 GitHub Releases 下载预编译的二进制文件。

> 本 Fork 当前发布多架构 Docker 镜像，但不一定提供 GitHub Release。安装脚本只会从 `rulanup/sub2api` 下载，不会回退安装上游官方版本。若 Fork 暂无 Release，请使用下方 Docker Compose 部署。
...
Fork 部署文件统一使用 `ghcr.io/rulanup/sub2api:latest`，支持 `linux/amd64` 和 `linux/arm64`。
