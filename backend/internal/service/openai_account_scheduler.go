...
func (s *OpenAIGatewayService) selectMultiGroupAccountWithScheduler(
	...
	for _, group := range groups {
		...
		groupCtx = context.WithValue(groupCtx, openAIMultiGroupBypassContextKey{}, true)
		selection, decision, err := s.selectAccountWithScheduler(groupCtx, &gid, previousResponseID, sessionHash, requestedModel, excludedIDs, requiredTransport, requiredCapability, requiredImageCapability, requireCompact, platform, false, useUpstreamTokenCost)
...
func (s *OpenAIGatewayService) selectAccountWithSchedulerOnce(
	...
	if bypass, _ := ctx.Value(openAIMultiGroupBypassContextKey{}).(bool); !bypass {
		if apiKey := apiKeyFromContext(ctx); apiKey != nil && len(apiKey.GroupIDs) > 1 {
			selection, decision, selectedGroup, err := s.selectMultiGroupAccountWithScheduler(ctx, apiKey, previousResponseID, sessionHash, requestedModel, excludedIDs, requiredTransport, requiredCapability, requiredImageCapability, requireCompact, platform, useUpstreamTokenCost)
