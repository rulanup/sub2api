...
func TestContentModerationTypeSafeAllCategoriesAndImages(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		require.Len(t, request.Questions, len(typeSafeModerationRules))
		answers := map[string]any{}
		// fork 合并后 ContentModerationCategories() 含 qwen3guard 等扩展分类，
		// TypeSafe 引擎仅覆盖其自有规则集内的分类。
		for category := range typeSafeModerationRules {
...
func TestContentModerationTypeSafeFailureOpenAndKeyIsolation(t *testing.T) {
	...
	s := &ContentModerationService{repo: repo}
	cfg := defaultContentModerationConfig()
	cfg.Engine = "typesafe"
	cfg.BaseURL = server.URL
