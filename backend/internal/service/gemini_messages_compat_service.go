...
func (s *GeminiMessagesCompatService) ForwardNative(ctx context.Context, c *gin.Context, account *Account, originalModel string, action string, stream bool, body []byte) (*ForwardResult, error) {
	...
	if resp.StatusCode >= 400 {
			...
			case ErrorPolicySkipped:
				...
				if !account.IsCustomErrorCodesEnabled() {
					...
					if ruleStatus, clientBody, matched := applyErrorPassthroughRuleToGoogleJSON(c, account.Platform, resp.StatusCode, respBody); matched {
						...
						c.Data(ruleStatus, contentType, clientBody)
		...
		if clientStatus, clientBody, matched := applyErrorPassthroughRuleToGoogleJSON(c, account.Platform, resp.StatusCode, respBody); matched {
