...
func (s *OpenAIGatewayService) SelectAccountWithLoadAwareness(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}) (*AccountSelectionResult, error) {
	if apiKey := apiKeyFromContext(ctx); apiKey != nil && len(apiKey.GroupIDs) > 1 {
		...
		for _, group := range s.orderedAPIKeyScheduleGroups(ctx, apiKey, PlatformOpenAI) {
			...
			groupCtx = s.withOpenAIQuotaAutoPauseContext(groupCtx)
			groupCtx = s.withOpenAIProfitControlGate(groupCtx, &gid)
			selection, err := s.selectAccountWithLoadAwareness(groupCtx, &gid, PlatformOpenAI, sessionHash, requestedModel, excludedIDs, false, "", true)
