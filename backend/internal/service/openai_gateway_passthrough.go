...
func (s *OpenAIGatewayService) handleErrorResponsePassthrough(
	...
	if matched {
		...
		if contentType == "" || (!bytes.Equal(clientBody, body) && gjson.ValidBytes(clientBody)) {
			contentType = "application/json"
		}
		c.Data(clientStatus, contentType, clientBody)
	} else if isOpenAIContextWindowError(upstreamMsg, body) && upstreamMsg != "" {
		// Context-window errors are actionable and deterministic, so retain the
		// sanitized upstream message instead of replacing it with a generic one.
