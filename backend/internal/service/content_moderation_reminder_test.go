...
// fork 语义：OpenAI Chat 上下文以 assistant 结尾时（典型为 DSH 重发完整对话），
// 仍回溯审计最后一条用户消息，而不是跳过。
func TestExtractContentModerationKeywordText_ReauditsLatestUserWhenAssistantTerminates(t *testing.T) {
	body := []byte(`{"messages":[{"role":"user","content":"old"},{"role":"assistant","content":"今晚打老虎"}]}`)
	require.Equal(t, "old", extractContentModerationKeywordText(ContentModerationProtocolOpenAIChat, body))
}
