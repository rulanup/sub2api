...
func (s *APIKeyService) snapshotFromAPIKey(ctx context.Context, apiKey *APIKey) *APIKeyAuthSnapshot {
	...
	snapshot := &APIKeyAuthSnapshot{
		...
		GroupScheduleStrategy: apiKey.GroupScheduleStrategy,
		Name:                  apiKey.Name,
	...
	if len(apiKey.Groups) > 0 {
		...
		for _, group := range apiKey.Groups {
			...
			if snap := apiKeyAuthGroupSnapshotFromGroup(group); snap != nil {
...
func (s *APIKeyService) snapshotToAPIKey(key string, snapshot *APIKeyAuthSnapshot) *APIKey {
	...
	apiKey := &APIKey{
		...
		GroupScheduleStrategy: snapshot.GroupScheduleStrategy,
		Key:                   key,
		Name:                  snapshot.Name,
	...
	if len(snapshot.Groups) > 0 {
		...
		for i := range snapshot.Groups {
			apiKey.Groups = append(apiKey.Groups, groupFromAPIKeyAuthSnapshot(&snapshot.Groups[i]))
		}
	}
