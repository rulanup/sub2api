...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	lotteryActivityService := service.NewLotteryActivityService(lotteryActivityRepository, settingRepository, groupRepository, subscriptionService, billingCacheService, apiKeyAuthCacheInvalidator)
	...
	// Ensure checkin_records table exists
	_, _ = db.ExecContext(context.Background(), `CREATE TABLE IF NOT EXISTS checkin_records (id BIGSERIAL PRIMARY KEY, user_id BIGINT NOT NULL, amount DOUBLE PRECISION NOT NULL DEFAULT 0, checkin_date DATE NOT NULL, created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW(), UNIQUE(user_id, checkin_date)); CREATE INDEX IF NOT EXISTS idx_checkin_records_user_date ON checkin_records(user_id, checkin_date);`)
