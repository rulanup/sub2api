...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	privateAccountRepository := repository.NewPrivateAccountRepository(client, db)
	privateAccountService := service.NewPrivateAccountService(privateAccountRepository, groupRepository)
	privateAccountHandler := handler.NewPrivateAccountHandler(privateAccountService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, checkinHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, batchImageHandler, tokenRankingHandler, privateAccountHandler, idempotencyCoordinator, idempotencyCleanupService)
