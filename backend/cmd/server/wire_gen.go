...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	checkinService := service.NewCheckinService(checkinRepository, userRepository)
	...
	lotteryGroupReader := service.ProvideLotteryGroupReader(groupRepository)
	lotteryActivityService := service.NewLotteryActivityService(lotteryActivityRepository, settingRepository, lotteryGroupReader, subscriptionService, billingCacheService, apiKeyAuthCacheInvalidator)
	lotteryActivityHandler := handler.NewLotteryActivityHandler(lotteryActivityService)
	...
	userRiskProfileRepository := repository.NewUserRiskProfileRepository(db)
	userRiskScoreService := service.NewUserRiskScoreService(userRiskProfileRepository)
	...
	contentModerationService := service.ProvideContentModerationService(settingRepository, contentModerationRepository, contentModerationHashCache, groupRepository, userRepository, proxyRepository, apiKeyAuthCacheInvalidator, emailService, secretEncryptor, configConfig, leaderLockCache, db)
	...
	defaultAuditGate := securityaudit.ProvideDefaultAuditGate(contentModerationService)
	promptErrorService := securityaudit.NewGatedPromptErrorService(postgreSQLRepository, defaultAuditGate)
	promptErrorAdminHandler := securityaudit.NewPromptErrorAdminHandler(promptErrorService)
	paymentHandler := admin.NewPaymentHandler(paymentService, paymentConfigService)
	affiliateHandler := admin.NewAffiliateHandler(affiliateService, adminService)
	complianceHandler := admin.NewComplianceHandler(settingService)
	adminLotteryActivityHandler := admin.NewLotteryActivityHandler(lotteryActivityService)
	auditLogRepository := repository.NewAuditLogRepository(db)
	auditLogService := service.ProvideAuditLogService(auditLogRepository, settingService)
	auditLogHandler := admin.NewAuditLogHandler(auditLogService, totpService)
	upstreamBillingProbeService := service.ProvideUpstreamBillingProbeService(accountRepository, accountTestService, settingService, leaderLockCache, db)
	openCodeGoUsageService := service.ProvideOpenCodeGoUsageService(accountRepository, httpUpstream, settingService, leaderLockCache, db)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, userRiskScoreService, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, cnProviderHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, pluginHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, promptErrorAdminHandler, paymentHandler, affiliateHandler, complianceHandler, adminLotteryActivityHandler, auditLogHandler, upstreamBillingProbeService, ollamaCloudUsageService, openCodeGoUsageService)
	...
	coordinator := securityaudit.ProvideCoordinator(legacyEngine, promptService, userRiskScoreService, defaultAuditGate)
	gatewayHandler := handler.ProvideGatewayHandler(gatewayService, openAIGatewayService, geminiMessagesCompatService, antigravityGatewayService, userService, concurrencyService, billingCacheService, usageService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, userMessageQueueService, configConfig, settingService, coordinator, promptErrorService)
	openAIGatewayHandler := handler.ProvideOpenAIGatewayHandler(openAIGatewayService, pluginManager, concurrencyService, billingCacheService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, opsService, grokQuotaService, configConfig, coordinator, promptErrorService)
	...
	availableChannelHandler := handler.NewAvailableChannelHandler(channelService, apiKeyService, settingService, client)
	...
	userAccountHandler := handler.NewUserAccountHandler(accountHandler, client, accountTestService)
	userGroupHandler := handler.NewUserGroupHandler(client)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	openAIQuotaAutoResetService := service.ProvideOpenAIQuotaAutoResetService(accountRepository, openAIQuotaService, rateLimitService, idempotencyCoordinator, auditLogService, settingService, leaderLockCache)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, checkinHandler, lotteryActivityHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, tokenRankingHandler, userAccountHandler, userGroupHandler, idempotencyCoordinator, idempotencyCleanupService, openAIQuotaAutoResetService)
	...
	userDailySummaryService := service.ProvideUserDailySummaryService(userRepository, usageLogRepository, settingRepository, notificationEmailService, leaderLockCache, db)
