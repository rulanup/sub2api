  ...
  <div ref="containerRef" class="relative">
    <div v-if="selectedUserIds.length > 0" class="mb-2 flex flex-wrap gap-2">
      ...
      >
        ...
        <span v-if="selectedUsers[userId]?.deleted" class="shrink-0 text-gray-400">
          {{ deletedText }}
        </span>
        <button
          type="button"
...
const placeholderText = computed(() => props.placeholder ?? t('admin.riskControl.userAllowlistSearchPlaceholder'))
const emptyText = computed(() => props.emptyLabel ?? t('admin.riskControl.userAllowlistSearchEmpty'))
const deletedText = computed(() => props.deletedLabel ?? t('admin.riskControl.userAllowlistDeleted'))
const removeText = computed(() => props.removeLabel ?? t('admin.riskControl.userAllowlistRemove'))
...
function selectedUserLabel(userId: number): string {
  return selectedUsers.value[userId]?.email
    ?? props.fallbackLabel?.replace('{id}', String(userId))
    ?? t('admin.riskControl.userAllowlistFallback', { id: userId })
}
...
async function hydrateSelectedUsers(userIds: number[]): Promise<void> {
  ...
  const users = await Promise.all(missing.map(async (id) => {
    try {
      const user = await adminAPI.users.getById(id, true)
      return { id: user.id, email: user.email, deleted: Boolean(user.deleted_at) } satisfies SimpleUser
    } catch {
      return null
    }
  }))
  ...
  for (const user of users) {
    if (user && props.modelValue.includes(user.id)) next[user.id] = user
  }
...
watch(selectedUserIds, (userIds) => void hydrateSelectedUsers(userIds), { immediate: true })
