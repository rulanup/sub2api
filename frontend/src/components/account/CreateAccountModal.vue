...
interface Props {
  ...
  accountsApi?: Pick<typeof adminAPI.accounts, 'create' | 'checkMixedChannelRisk'> &
    Partial<Pick<typeof adminAPI.accounts, 'probeUpstreamBilling' | 'syncUpstreamModels'>>
}
...
const submitCreateAccount = async (payload: CreateAccountRequest) => {
  ...
  try {
    const account = await accountsAPI.value.create(withAntigravityConfirmFlag(payload))
    ...
    if ((upstreamModelsPreviewed.value || hasConcreteMappedTarget) && accountsAPI.value.syncUpstreamModels) {
      try {
        const result = await accountsAPI.value.syncUpstreamModels(account.id)
    ...
    if (
      ...
      accountsAPI.value.probeUpstreamBilling
    ) {
      try {
        await accountsAPI.value.probeUpstreamBilling(account.id)
      } catch {
