<template>
  ...
  >
    ...
    >
      ...
      <div
        v-if="isGrokOAuthAccount && grokMediaEligibilitySupported"
        class="border-t border-gray-200 pt-4 dark:border-dark-600"
...
type AccountEditAPI = Pick<typeof adminAPI.accounts, 'update' | 'checkMixedChannelRisk'> &
  Partial<Pick<typeof adminAPI.accounts, 'getGrokMediaEligibility' | 'updateGrokMediaEligibility'>>
...
const grokMediaEligibilitySupported = computed(
  () =>
    typeof accountsAPI.value.getGrokMediaEligibility === 'function' &&
    typeof accountsAPI.value.updateGrokMediaEligibility === 'function'
)
...
const loadGrokMediaEligibility = async (accountID: number): Promise<GrokMediaEligibilityState | null> => {
  const getGrokMediaEligibility = accountsAPI.value.getGrokMediaEligibility
  if (!isGrokOAuthAccount.value || typeof getGrokMediaEligibility !== 'function') {
    return null
  }
  ...
  try {
    const state = await getGrokMediaEligibility(accountID)
...
const persistGrokMediaEligibility = async (accountID: number, updatedAccount: Account): Promise<Account> => {
  const updateGrokMediaEligibility = accountsAPI.value.updateGrokMediaEligibility
  if (
    !isGrokOAuthAccount.value ||
    grokMediaEligibilityMode.value === grokMediaEligibilityInitialMode.value ||
    typeof updateGrokMediaEligibility !== 'function'
  ) {
    return updatedAccount
  }

  try {
    const state = await updateGrokMediaEligibility(accountID, grokMediaEligibilityMode.value)
    grokMediaEligibilityState.value = state
    grokMediaEligibilityInitialMode.value = state.mode
...
const submitUpdateAccount = async (accountID: number, updatePayload: Record<string, unknown>) => {
  ...
  try {
    let updatedAccount = await accountsAPI.value.update(accountID, withAntigravityConfirmFlag(updatePayload))
