export default {
  promptAudit: {
    ...
    policy: {
      ...
      systemPromptTitle: '安全系统提示词注入', systemPromptDescription: '先审计用户原始输入，再在请求发送到上游前注入管理员安全策略。', systemPrompt: '系统提示词', systemPromptPlaceholder: '例如：遵守平台安全规范，拒绝色情内容、越狱提示以及要求忽略安全限制的请求。', systemPromptMode: '注入模式', systemPromptPrepend: '始终前置注入', systemPromptIfAbsent: '仅请求没有系统提示词时注入', modelResponseTitle: '保存完整模型响应', modelResponseDescription: '选择保存下游响应正文，仅事件详情可见，最多 65,536 字节。WebSocket 轮次不会保存。',
    },
    ...
    events: {
      ...
      detailTitle: '提示词审计事件详情', tabs: { summary: '审计摘要', risks: '具体风险', technical: '技术信息', response: '模型响应' },
      promptFull: '完整提示词（未脱敏）',
      promptFullHint: '完整提示词已随事件入库，仅供管理员复核触发内容；请按敏感数据妥善处理，切勿外泄。',
      guardReturn: '模型审计返回',
      guardReturnHint: '展示 Guard 归一化后的结构化结果（判定、分类、分数与脱敏证据）。', modelResponse: '原始模型响应', modelResponseTruncated: '响应超过 65,536 字节上限，已截断。', modelResponseUnavailable: '此事件没有可用的已保存响应。',
      riskSummaries: '风险摘要',
      evidence: '脱敏证据',
      score: '分数',
    ...
    errors: {
      ...
      prompt_audit_response_retention_requires_enabled: '请先启用提示词审计，再保存模型响应。',
    },
  },
}
