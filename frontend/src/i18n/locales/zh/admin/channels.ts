export default {
    ...
    channels: {
      ...
      form: {
        ...
        searchAccountForModels: '搜索账号',
        fetchModelsFromAccount: '从账号获取',
        fetchingAccountModels: '获取中',
        accountModelsAlreadyAdded: '该账号没有可添加的模型白名单条目',
        fetchAccountModelsError: '获取账号模型失败',
    ...
    riskControl: {
      ...
      protocolAliyunGuardrails: '阿里云 AI 安全护栏',
      openaiModerationProtocolHint: '使用 /v1/moderations 接口返回分类分数。',
      qwenProtocolHint: '使用 /v1/chat/completions 接口解析 Safety 与 Categories。',
      aliyunProtocolHint: '使用阿里云内容安全 AI 安全护栏服务审核文本输入。',
      controversialAction: 'Controversial 处理方式',
      controversialActionAllow: '放行并记录',
      controversialActionBlock: '拦截请求',
      controversialActionHint: '争议内容默认放行并记录；选择拦截后会在主请求发送前拒绝。',
      qwenModelHint: 'Gitee AI 默认模型为 Qwen3Guard-Gen-8B，可按节点实际模型名修改。',
      qwenImageUnsupportedHint: 'Qwen3Guard Chat 首版仅审核文本；图片输入会提示审计节点不支持并按故障放行。',
      aliyunRegion: '地域',
      aliyunRegionShanghai: '华东 2（上海）',
      aliyunRegionBeijing: '华北 2（北京）',
      aliyunRegionHangzhou: '华东 1（杭州）',
      aliyunRegionShenzhen: '华南 1（深圳）',
      aliyunRegionChengdu: '西南 1（成都）',
      aliyunRegionSingapore: '新加坡',
      aliyunRegionFrankfurt: '德国（法兰克福）',
      aliyunService: '检测服务',
      aliyunServiceHint: '默认使用增强版文本安全检测 query_security_check_pro。',
      aliyunEndpoint: 'Endpoint',
      aliyunCredentials: '阿里云访问凭证',
      aliyunCredentialsHint: '留空会保留并复用已保存凭证；输入新值可替换对应字段。Secret 仅在本次请求中提交。',
      aliyunAccessKeyId: 'AccessKey ID',
      aliyunAccessKeyIdPlaceholder: '输入 AccessKey ID；留空保持不变',
      aliyunAccessKeyIdHint: '已保存的 ID 仅以掩码显示，不会回显完整值。',
      aliyunAccessKeySecret: 'AccessKey Secret',
      aliyunAccessKeySecretPlaceholder: '输入新 Secret；留空保持不变',
      aliyunAccessKeySecretHint: 'Secret 不会回显，提交保存或测试后会立即清空输入框。',
      aliyunCredentialsConfigured: '凭证已配置',
      aliyunCredentialsConfiguredMasked: '凭证已配置：{id}',
      aliyunCredentialsPendingClear: '待清除',
      clearAliyunCredentials: '清除已保存凭证',
      keepAliyunCredentials: '保留已保存凭证',
      aliyunCredentialsWillClear: '保存后将同时清除 AccessKey ID 和 Secret。',
      aliyunTextOnlyTitle: '首期仅支持文本输入',
      aliyunTextOnlyHint: '图片会跳过阿里云外部审核，并在审核记录中记为错误。',
      testAliyunConnection: '测试连接',
      testingAliyunConnection: '测试中',
      aliyunTestHint: 'ID 与 Secret 均留空时复用已保存凭证。',
      aliyunConnectionTestDone: '阿里云安全护栏连接测试完成',
      aliyunConnectionTestFailed: '阿里云安全护栏连接测试失败',
      baseUrl: '审核节点 Base URL',
      model: '模型名',
      ...
      auditTestSeverity: '模型判定：{severity}',
      auditTestCategories: '模型分类',
      auditSeveritySafe: 'Safe（安全）',
      auditSeverityUnsafe: 'Unsafe（不安全）',
      auditSeverityControversial: 'Controversial（争议）',
      auditEndpointUnsupported: '当前审计节点不支持该接口，请检查协议配置或更换支持 Chat Completions 的节点。',
      ...
      syncAbuseTitle: '同步中转检测',
      syncAbuseRule: '使用固定 10 分钟窗口检测持续中转：窗口内每个完整分钟均至少有 1 次同步请求时判定命中。',
      syncAbuseRpmLimit: '命中后设置 RPM',
      syncAbuseConcurrency: '命中后设置并发',
      syncAbuseDisableUser: '命中后禁用用户',
      syncAbuseDisableUserHint: '关闭时仅设置上方 RPM 和并发；开启后同时禁用对应用户账号。',
      syncAbuseAllowlistPlaceholder: '搜索不参与同步中转检测的用户',
      syncAbuseAllowlistHint: '选中的用户不会参与同步中转检测。',
      cyberUsageTitle: '安全策略用量检测',
      cyberUsageRule: '在配置的时间窗口内累计安全策略用量，达到阈值后禁用用户。',
      cyberUsageBanThreshold: '禁用阈值',
      cyberUsageWindowHours: '检测窗口（小时）',
      cyberUsageAllowlistPlaceholder: '搜索不参与安全策略用量检测的用户',
      cyberUsageAllowlistHint: '选中的用户不会参与安全策略用量检测。',
      userAllowlist: '用户白名单',
      userAllowlistSearchPlaceholder: '按邮箱搜索用户',
      userAllowlistSearchEmpty: '未找到用户',
      userAllowlistDeleted: '（已删除）',
      userAllowlistFallback: '用户 #{id}',
      userAllowlistRemove: '移除用户',
      ...
      riskThresholds: '风险阈值',
      riskThresholdsHint: '按审核分类调整命中阈值，适用于 OpenAI Moderations 和 Qwen3Guard；分数达到或超过阈值即视为命中。',
