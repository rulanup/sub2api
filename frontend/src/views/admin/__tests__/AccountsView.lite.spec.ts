...
vi.mock('@/api/user/accounts', () => ({
  userAccountsAPI: { list: listUserAccounts, getById: getUserAccount }
}))

vi.mock('@/api/user/groups', () => ({
  userPrivateGroupsAPI: { listAll: getAllGroups }
}))
...
function mountView(userMode = false, stubActionMenu = true) {
...
describe('admin AccountsView lite account list', () => {
  beforeEach(() => {
    ...
    listUserAccounts.mockReset().mockResolvedValue({ items: [listRow], total: 1, page: 1, page_size: 20, pages: 1 })
    getUserAccount.mockReset().mockResolvedValue({ ...fullAccount, name: 'private account details' })
  ...
  it('keeps the action menu open during internal scrolling but closes it on table scrolling', async () => {
    const wrapper = mountView(false, false)
    await flushPromises()
  ...
  it('loads private-account details through the user API before editing', async () => {
    ...
    expect(wrapper.get('[data-test="edit-account"]').text()).toBe('private account details')
    expect(getUserAccount).toHaveBeenCalledWith(42)
