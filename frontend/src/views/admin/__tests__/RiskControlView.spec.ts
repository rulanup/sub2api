...
describe('admin RiskControlView', () => {
  ...
  it('preserves both engine drafts and saves thresholds independently', async () => {
    ...
    await flushPromises()
    await findButtonByText(wrapper, 'admin.riskControl.openSettings').trigger('click')
    await wrapper.get('[data-test="moderation-base-url"]').setValue('https://openai-edited.example')
    ...
    await flushPromises()
    expect((wrapper.get('[data-test="moderation-model"]').element as HTMLInputElement).value).toBe('jev-latest')
    await wrapper.get('[data-test="moderation-base-url"]').setValue('https://typesafe-edited.example')
    await findButtonByText(wrapper, 'admin.riskControl.tabs.riskThresholds').trigger('click')
    ...
    await flushPromises()
    expect((wrapper.get('[data-test="moderation-base-url"]').element as HTMLInputElement).value).toBe('https://openai-edited.example')
    await findButtonByText(wrapper, 'admin.riskControl.saveConfig').trigger('click')
    await flushPromises()
