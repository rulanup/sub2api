      ...
      <template #filters>
        ...
        >
          ...
          >
            ...
            <button
              v-if="!userMode && !authStore.isSimpleMode"
              @click="openSortModal"
              class="btn btn-secondary"
        ...
        >
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              ...
              <button
                v-if="!userMode && !authStore.isSimpleMode && row.platform === 'composite'"
...
const loadModelAllowlistCandidates = async (
  ...
  if (userMode.value || authStore.isSimpleMode) return;
...
const openCreateModal = () => {
  ...
  if (!userMode.value && !authStore.isSimpleMode) {
    loadModelAllowlistCandidates("create", 0, createForm.platform);
  }
  if (!authStore.isSimpleMode) loadDirectAccountCandidates("create");
};
...
const handleCreateGroup = async () => {
  ...
  try {
    ...
    let createdGroup: AdminGroup;
    if (userMode.value) {
      createdGroup = await userPrivateGroupsAPI.create(requestData);
    } else {
    ...
    if (!authStore.isSimpleMode && createSelectedAccountIds.value.length > 0) {
...
const handleEdit = async (group: AdminGroup) => {
  ...
  if (!authStore.isSimpleMode) loadDirectAccountCandidates("edit");
...
const handleUpdateGroup = async () => {
  ...
  try {
    ...
    if (userMode.value) {
      await userPrivateGroupsAPI.update(editingGroup.value.id, payload);
    } else {
    ...
    if (!authStore.isSimpleMode && editSelectedAccountIds.value.length > 0) {
