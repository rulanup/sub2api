          ...
          <div v-if="activeSettingsTab === 'basic'" class="space-y-5">
            <div class="grid grid-cols-1 gap-5 lg:grid-cols-2">
              ...
              <div>
                ...
                <input v-model.trim="configForm.base_url" type="url" class="input" :placeholder="configForm.engine === 'typesafe' ? 'https://api.typesafe.ai' : protocolDefaultEndpoint" data-test="moderation-base-url" />
              </div>
              <div v-if="!isAliyunProtocol">
                <label class="input-label">{{ t('admin.riskControl.model') }}</label>
                <input v-model.trim="configForm.model" type="text" class="input" data-test="moderation-model" :placeholder="configForm.engine === 'typesafe' ? 'jev-latest' : configForm.protocol === 'qwen3guard_chat' ? 'Qwen3Guard-Gen-8B' : 'omni-moderation-latest'" />
                <p v-if="configForm.engine !== 'typesafe' && configForm.protocol === 'qwen3guard_chat'" class="mt-2 text-xs leading-5 text-gray-500 dark:text-gray-400">{{ t('admin.riskControl.qwenModelHint') }}</p>
              </div>
              <div>
