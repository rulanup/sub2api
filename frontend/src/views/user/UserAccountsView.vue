    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div class="w-full max-w-lg rounded-xl bg-white p-6 shadow-xl dark:bg-dark-800">
        ...
        <div class="mt-4 space-y-4">
          <div>
            <label class="input-label">{{ t('userAccounts.name') }}</label>
          ...
          <div>
            ...
            <input v-model="form.api_key" type="password" class="input font-mono" placeholder="sk-..." />
          </div>
...
const loadAccounts = async () => {
  loading.value = true
  try {
    const { data } = await apiClient.get('/user/accounts')
...
const loadGroups = async () => {
  try {
    const { data } = await apiClient.get('/admin/groups', { params: { lite: true } })
    availableGroups.value = data?.data || data || []
  } catch {
...
const saveAccount = async () => {
  ...
  try {
    ...
    if (form.value.api_key) {
      payload.credentials = { api_key: form.value.api_key }
    }
