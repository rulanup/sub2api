...
const routes: RouteRecordRaw[] = [
  ...
  },
  {
    ...
    meta: {
      ...
      descriptionKey: 'activity.subtitle'
    }
  },
  {
    path: '/affiliate',
...
router.beforeEach(async (to, _from, next) => {
  ...
  if ((to.meta.requiresPayment || to.meta.requiresRiskControl || to.meta.requiresActivity || to.meta.requiresSubscription) && !appStore.publicSettingsLoaded) {
    try {
      await appStore.fetchPublicSettings()
    } catch (error) {
      console.warn('Failed to load public settings in route guard', error)
  ...
  if (
    to.meta.requiresActivity &&
    appStore.publicSettingsLoaded &&
    appStore.cachedPublicSettings?.activity_enabled === false
  ) {
    next(authStore.isAdmin ? '/admin/dashboard' : '/dashboard')
    return
  }
