...
- **Resend メール配信** - 検証コードや通知メールを SMTP の代わりに Resend HTTPS API（resend.com）経由で送信でき、`GET /domains` による接続テストに対応します
- **メール設定の移行** - メール設定一式（SMTP + Resend）を JSON ファイルとしてワンクリックでエクスポート/インポートできます。既定では機密情報を含めず、インポート時はメール関連キーのみを反映します
...
- **システム設定 > メール** で配信プロバイダーを選択します：**SMTP** または **Resend API**。Resend は Resend ダッシュボードで作成したドメイン API Key が必要で、送信元ドメインが検証済みかつ送信有効である必要があります。
- Resend カードの **接続テスト** は `GET /domains` で API Key を検証するだけで、メールは送信されません。**テストメール送信** で受信確認ができます。
- メール送信プロバイダーカードの **設定をエクスポート** で、メール設定一式（SMTP + Resend）を JSON ファイルとしてダウンロードできます。SMTP パスワードと Resend API Key は既定で除外され、別環境への移行時のみ **機密情報を含める** にチェックしてください。
- **設定をインポート** でファイルから復元できます：メール関連キーのみが反映され、それ以外のキーは無視されます。機密フィールドが空の場合は既存値が保持されます。
